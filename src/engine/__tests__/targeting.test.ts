...
const P1_WITH_ZERA: [HeroId, HeroId, HeroId] = ["fire-mage", "zera", "water-healer"];
...
describe("target validation", () => {
  ...
  it("rejects a single-enemy card aimed at an ally", () => {
    const state = createMatch(P1, P2, createSeededRng(1));
    const cardId = putInHand(state, "player1", "fire-bolt");
    const ally = heroInstanceId("player1", "cragor");
  ...
  it("rejects a single-ally card aimed at an enemy", () => {
    const state = createMatch(P1, P2, createSeededRng(1));
    const cardId = putInHand(state, "player1", "fortress-stance");
  ...
  it("requires a distinct secondary target for Twin Volt when 2+ enemies are alive", () => {
    const state = createMatch(P1_WITH_ZERA, P2, createSeededRng(1));
    const cardId = putInHand(state, "player1", "twin-volt");
    const primary = heroInstanceId("player2", "zera");
    expect(() => queueCard(state, "player1", cardId, { primaryTargetId: primary })).toThrow(
      /second/i,
    );
  });

  it("allows Twin Volt with just a primary target once only one enemy remains", () => {
    const state = createMatch(P1_WITH_ZERA, P2, createSeededRng(1));
